...
- [PNG 无损智能分割器 v2.0](tools/png-lossless-splitter/)：浏览器离线工具，可自动识别多个透明 PNG 图块、统一输出尺寸、智能删除底部文字、手动矩形/画笔删除，并保持原始 Alpha 抗锯齿边缘。该子工具单独采用 MIT License。

## 许可

当前仓库主程序尚未附加开源许可证；公开可见不等同于授予复制、修改或再分发许可。`tools/png-lossless-splitter/` 子工具除外，该目录按其内 `LICENSE` 使用 MIT License。
